package
{
	...
	public class Level3 extends FlxState
	{
		...
		{
			map = new FlxTilemap();
			map.loadMap(new Sources.TxtMap3, Sources.ImgMap, 16, 16);
			add(map);
			lava = new FlxGroup;
			door = new FlxSprite(map.width - 16, map.height - 80 - 16, Sources.ImgDoor);
			add(door);
			key = new FlxSprite(map.width - 514, map.height - 160 + 16, Sources.ImgKey);
			add(key);
			player = new Player;			
			player.x = map.width - 2000;
			player.y = FlxG.height - 48;
			add(player);			
			FlxG.resetCameras();
